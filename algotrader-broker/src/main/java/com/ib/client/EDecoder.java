...
class EDecoder implements ObjectInput {
  ...
  private final EClientMsgSink m_clientMsgSink;
  ...
  public int processMsg(EMessage msg) throws IOException {
    ...
    switch (msgId) {
      ...
      default: {
        m_EWrapper.error(EClientErrors.NO_VALID_ID, EClientErrors.UNKNOWN_ID.code(),
            EClientErrors.UNKNOWN_ID.msg(), null);
        return 0;
      }
    }
  ...
  private void processHistoricalTicksLast() throws IOException {
    ...
    for (int i = 0; i < tickCount; i++) {
      ...
      ticks.add(
          new HistoricalTickLast(time, tickAttribLast, price, size, exchange, specialConditions));
    }
  ...
  private void processHistoricalTicksBidAsk() throws IOException {
    ...
    for (int i = 0; i < tickCount; i++) {
      ...
      ticks.add(
          new HistoricalTickBidAsk(time, tickAttribBidAsk, priceBid, priceAsk, sizeBid, sizeAsk));
    }
  ...
  private void processHistoricalDataUpdateMsg() throws IOException {
    ...
    m_EWrapper.historicalDataUpdate(reqId,
    ...
    if (nDepthMktDataDescriptions > 0) {
      ...
      for (int i = 0; i < nDepthMktDataDescriptions; i++) {
        if (m_serverVersion >= EClient.MIN_SERVER_VER_SERVICE_DATA_TYPE) {
          depthMktDataDescriptions[i] = new DepthMktDataDescription(readStr(), readStr(), readStr(),
              readStr(), readIntMax());
        } else {
          depthMktDataDescriptions[i] = new DepthMktDataDescription(readStr(), readStr(), "",
              readBoolFromInt() ? "Deep2" : "Deep", Integer.MAX_VALUE);
        }
      }
    }
  ...
  private void processSecurityDefinitionOptionalParameterMsg() throws IOException {
    ...
    m_EWrapper.securityDefinitionOptionalParameter(reqId, exchange, underlyingConId, tradingClass,
  ...
  private void processDeltaNeutralValidationMsg() throws IOException {
    ...
    DeltaNeutralContract deltaNeutralContract = new DeltaNeutralContract(readInt(), readDouble(),
        readDouble());
  ...
  private void processHistoricalDataMsg() throws IOException {
    ...
    for (int ctr = 0; ctr < itemCount; ctr++) {
      ...
      m_EWrapper.historicalData(reqId,
          new Bar(date, open, high, low, close, volume, barCount, WAP));
    }
  ...
  private void processContractDataMsg() throws IOException {
    ...
    if (version >= 5) {
      contract.longName(
          m_serverVersion >= EClient.MIN_SERVER_VER_ENCODE_MSG_ASCII7 ? decodeUnicodeEscapedString(
              readStr()) : readStr());
  ...
  private void processOpenOrderMsg() throws IOException {
    ...
    // read version
    int version =
        m_serverVersion < EClient.MIN_SERVER_VER_ORDER_CONTAINER ? readInt() : m_serverVersion;

    Contract contract = new Contract();
    Order order = new Order();
    OrderState orderState = new OrderState();
    EOrderDecoder eOrderDecoder = new EOrderDecoder(this, contract, order, orderState, version,
        m_serverVersion);
  ...
  private void processOrderStatusMsg() throws IOException {
    int version =
        m_serverVersion >= EClient.MIN_SERVER_VER_MARKET_CAP_PRICE ? Integer.MAX_VALUE : readInt();
    int id = readInt();
  ...
  private void processTickOptionComputationMsg() throws IOException {
    int version =
        m_serverVersion >= EClient.MIN_SERVER_VER_PRICE_BASED_VOLATILITY ? Integer.MAX_VALUE
            : readInt();
    int tickerId = readInt();
    int tickType = readInt();
    ...
    m_EWrapper.tickOptionComputation(tickerId, tickType, tickAttrib, impliedVol, delta, optPrice,
  ...
  private void processTickByTickMsg() throws IOException {
    ...
    switch (tickType) {
      ...
      case 2: // AllLast
        ...
        m_EWrapper.tickByTickAllLast(reqId, tickType, time, price, size, tickAttribLast, exchange,
            specialConditions);
        break;
      case 3: // BidAsk
        ...
        m_EWrapper.tickByTickBidAsk(reqId, time, bidPrice, askPrice, bidSize, askSize,
            tickAttribBidAsk);
        break;
      case 4: // MidPoint
  ...
  private void processCompletedOrderMsg() throws IOException {
    ...
    EOrderDecoder eOrderDecoder = new EOrderDecoder(this, contract, order, orderState,
        Integer.MAX_VALUE, m_serverVersion);
  ...
  private void readLastTradeDate(ContractDetails contract, boolean isBond) throws IOException {
    ...
    if (lastTradeDateOrContractMonth != null) {
      String[] splitted =
          lastTradeDateOrContractMonth.contains("-") ? lastTradeDateOrContractMonth.split("-")
