...
public class EOrderDecoder {
  ...
  EOrderDecoder(EDecoder eDecoder, Contract contract, Order order, OrderState orderState,
  ...
  public void readScaleOrderParams() throws IOException {
    ...
    if (m_version >= 28 && m_order.scalePriceIncrement() > 0.0
        && m_order.scalePriceIncrement() != Double.MAX_VALUE) {
  ...
  public void readSoftDollarTier() throws IOException {
    if (m_serverVersion >= EClient.MIN_SERVER_VER_SOFT_DOLLAR_TIER) {
      m_order.softDollarTier(
          new SoftDollarTier(m_eDecoder.readStr(), m_eDecoder.readStr(), m_eDecoder.readStr()));
    }
  }
