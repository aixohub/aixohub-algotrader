...
public class EWrapperMsgGenerator {
  ...
  public static String tickPrice(int tickerId, int field, double price, TickAttrib attribs) {
    return "id=" + tickerId + "  " + TickType.getField(field) + "=" + Util.DoubleMaxString(price)
        + " " +
        (attribs.canAutoExecute() ? " canAutoExecute" : " noAutoExecute") + " pastLimit = "
        + attribs.pastLimit() +
        (field == TickType.BID.index() || field == TickType.ASK.index() ? " preOpen = "
            + attribs.preOpen() : "");
  }
  ...
  public static String tickOptionComputation(int tickerId, int field, int tickAttrib,
      double impliedVol,
  ...
  public static String tickGeneric(int tickerId, int tickType, double value) {
    return "id=" + tickerId + "  " + TickType.getField(tickType) + "=" + Util.DoubleMaxString(
        value);
  }
  ...
  public static String tickEFP(int tickerId, int tickType, double basisPoints,
    ...
    return "id=" + tickerId + "  " + TickType.getField(tickType)
        ...
        + " impliedFuture = " + Util.DoubleMaxString(impliedFuture) + " holdDays = "
        + Util.IntMaxString(holdDays) +
        " futureLastTradeDate = " + futureLastTradeDate + " dividendImpact = "
  ...
  public static String orderStatus(int orderId, String status, Decimal filled, Decimal remaining,
    ...
    return "order status: orderId=" + orderId + " clientId=" + Util.IntMaxString(clientId)
        + " permId=" + Util.IntMaxString(permId) +
        " status=" + status + " filled=" + filled + " remaining=" + remaining +
        " avgFillPrice=" + Util.DoubleMaxString(avgFillPrice) + " lastFillPrice="
        + Util.DoubleMaxString(lastFillPrice) +
        " parent Id=" + Util.IntMaxString(parentId) + " whyHeld=" + whyHeld + " mktCapPrice="
        + Util.DoubleMaxString(mktCapPrice);
  }

  public static String openOrder(int orderId, Contract contract, Order order,
  ...
  public static String updateAccountValue(String key, String value, String currency,
      String accountName) {
  ...
  public static String updatePortfolio(Contract contract, Decimal position, double marketPrice,
    ...
    return "updatePortfolio: "
        ...
        + position + " " + Util.DoubleMaxString(marketPrice) + " " + Util.DoubleMaxString(
        marketValue) + " "
  ...
  public static String updateMktDepth(int tickerId, int position, int operation, int side,
    ...
    return "updateMktDepth: " + tickerId + " " + position + " " + operation + " " + side + " "
        + Util.DoubleMaxString(price) + " " + size;
  }
  ...
  public static String updateMktDepthL2(int tickerId, int position, String marketMaker,
    ...
    return "updateMktDepth: " + tickerId + " " + position + " " + marketMaker + " " + operation
        + " " + side + " " + Util.DoubleMaxString(price) + " " + size + " " + isSmartDepth;
  }

  public static String updateNewsBulletin(int msgId, int msgType, String message,
      String origExchange) {
    return "MsgId=" + msgId + " :: MsgType=" + Util.IntMaxString(msgType) + " :: Origin="
  ...
  public static String deltaNeutralValidation(int reqId,
  ...
  public static String accountSummary(int reqId, String account, String tag, String value,
      String currency) {
  ...
  public static String positionMulti(int reqId, String account, String modelCode, Contract contract,
  ...
  public static String accountUpdateMulti(int reqId, String account, String modelCode, String key,
  ...
  public static String securityDefinitionOptionalParameter(int reqId, String exchange,
  ...
  public static String softDollarTiers(int reqId, SoftDollarTier[] tiers) {
    ...
    sb.append("==== Soft Dollar Tiers Begin (total=").append(tiers.length).append(") reqId: ")
        .append(reqId).append(" ====\n");
  ...
  public static String symbolSamples(int reqId, ContractDescription[] contractDescriptions) {
    StringBuilder sb = new StringBuilder(256);
    sb.append("==== Symbol Samples Begin (total=").append(contractDescriptions.length)
    ...
    for (int i = 0; i < contractDescriptions.length; i++) {
      ...
      sb.append("primaryExch: ").append(contractDescriptions[i].contract().primaryExch())
          .append("\n");
      sb.append("currency: ").append(contractDescriptions[i].contract().currency()).append("\n");
      sb.append("derivativeSecTypes (total=")
          .append(contractDescriptions[i].derivativeSecTypes().length).append("): ");
      ...
      sb.append("description: ")
          .append(Util.NormalizeString(contractDescriptions[i].contract().description()))
          .append("\n");
      sb.append("issuerId: ")
          .append(Util.NormalizeString(contractDescriptions[i].contract().issuerId())).append("\n");
      sb.append("---- Contract Description End (").append(i).append(") ----\n");
    }
    sb.append("==== Symbol Samples End (total=").append(contractDescriptions.length)
        .append(") reqId: ").append(reqId).append(" ====\n");
  ...
  public static String mktDepthExchanges(DepthMktDataDescription[] depthMktDataDescriptions) {
    StringBuilder sb = new StringBuilder();
    sb.append("==== Market Depth Exchanges Begin (total=").append(depthMktDataDescriptions.length)
        .append(") ====\n");
    for (int i = 0; i < depthMktDataDescriptions.length; i++) {
      sb.append("Depth Market Data Description [").append(i).append("] - exchange: ")
          .append(depthMktDataDescriptions[i].exchange())
          ...
          .append(", aggGroup: ").append(Util.IntMaxString(depthMktDataDescriptions[i].aggGroup()))
          .append("\n");
    }
    sb.append("==== Market Depth Exchanges End (total=").append(depthMktDataDescriptions.length)
        .append(") ====\n");
    return sb.toString();
  }

  public static String tickNews(int tickerId, long timeStamp, String providerCode, String articleId,
      String headline, String extraData) {
    return "TickNews. tickerId: " + tickerId + ", timeStamp: " + Util.UnixMillisecondsToString(
        timeStamp, "yyyyMMdd-HH:mm:ss") +
        ", providerCode: " + providerCode + ", articleId: " + articleId + ", headline: " + headline
        + ", extraData: " + extraData;
  }
  ...
  public static String newsProviders(NewsProvider[] newsProviders) {
    ...
    for (int i = 0; i < newsProviders.length; i++) {
      sb.append("News Provider [").append(i).append("] - providerCode: ")
          .append(newsProviders[i].providerCode()).append(", providerName: ")
  ...
  public static String tickReqParams(int tickerId, double minTick, String bboExchange,
      int snapshotPermissions) {
    return "id=" + tickerId + " minTick = " + Util.DoubleMaxString(minTick) + " bboExchange = "
        + bboExchange + " snapshotPermissions = " + Util.IntMaxString(snapshotPermissions);
  }

  public static String smartComponents(int reqId, Map<Integer, Entry<String, Character>> theMap) {
    StringBuilder sb = new StringBuilder();

    sb.append("==== Smart Components Begin (total=").append(theMap.entrySet().size())
        .append(") reqId = ").append(reqId).append("====\n");

    for (Map.Entry<Integer, Entry<String, Character>> item : theMap.entrySet()) {
      sb.append("bit number: ").append(item.getKey()).append(", exchange: ")
          .append(item.getValue().getKey()).append(", exchange letter: ")
          .append(item.getValue().getValue()).append("\n");
    }

    sb.append("==== Smart Components End (total=").append(theMap.entrySet().size())
        .append(") reqId = ").append(reqId).append("====\n");
  ...
  public static String historicalNews(int requestId, String time, String providerCode,
      String articleId, String headline) {
    return "Historical News. RequestId: " + requestId + ", time: " + time + ", providerCode: "
        + providerCode +
  ...
  public static String histogramData(int reqId, List<HistogramEntry> items) {
    StringBuilder sb = new StringBuilder();
    sb.append("Histogram data. Req Id: ").append(reqId).append(", Data (").append(items.size())
        .append("):\n");
    items.forEach(
        i -> sb.append("\tPrice: ").append(Util.DoubleMaxString(i.price())).append(", Size: ")
            .append(i.size()).append("\n"));
    return sb.toString();
  }

  public static String rerouteMktDataReq(int reqId, int conId, String exchange) {
    return "Re-route market data request. Req Id: " + reqId + ", Con Id: " + conId + ", Exchange: "
        + exchange;
  }

  public static String rerouteMktDepthReq(int reqId, int conId, String exchange) {
    return "Re-route market depth request. Req Id: " + reqId + ", Con Id: " + conId + ", Exchange: "
        + exchange;
  }
  ...
  public static String pnl(int reqId, double dailyPnL, double unrealizedPnL, double realizedPnL) {
    return "Daily PnL. Req Id: " + reqId + ", daily PnL: " + Util.DoubleMaxString(dailyPnL)
        + ", unrealizedPnL: " + Util.DoubleMaxString(unrealizedPnL) + ", realizedPnL: "
        + Util.DoubleMaxString(realizedPnL);
  }

  public static String pnlSingle(int reqId, Decimal pos, double dailyPnL, double unrealizedPnL,
      double realizedPnL, double value) {
    return "Daily PnL Single. Req Id: " + reqId + ", pos: " + pos + ", daily PnL: "
        + Util.DoubleMaxString(dailyPnL) + ", unrealizedPnL: " + Util.DoubleMaxString(unrealizedPnL)
        + ", realizedPnL: " + Util.DoubleMaxString(realizedPnL) + ", value: "
        + Util.DoubleMaxString(value);
  }

  public static String historicalTick(int reqId, long time, double price, Decimal size) {
    return "Historical Tick. Req Id: " + reqId + ", time: " + Util.UnixSecondsToString(time,
        "yyyyMMdd-HH:mm:ss") + ", price: " + Util.DoubleMaxString(price) + ", size: "
        + size;
  }

  public static String historicalTickBidAsk(int reqId, long time, TickAttribBidAsk tickAttribBidAsk,
      double priceBid, double priceAsk,
      Decimal sizeBid, Decimal sizeAsk) {
    return "Historical Tick Bid/Ask. Req Id: " + reqId + ", time: " + Util.UnixSecondsToString(time,
        "yyyyMMdd-HH:mm:ss") + ", bid price: " + Util.DoubleMaxString(priceBid)
        + ", ask price: " + Util.DoubleMaxString(priceAsk) + ", bid size: " + sizeBid
        + ", ask size: " + sizeAsk
        + ", tick attribs: " + (tickAttribBidAsk.bidPastLow() ? "bidPastLow " : "") + (
        tickAttribBidAsk.askPastHigh() ? "askPastHigh " : "");
  }

  public static String historicalTickLast(int reqId, long time, TickAttribLast tickAttribLast,
      double price, Decimal size, String exchange,
      String specialConditions) {
    return "Historical Tick Last. Req Id: " + reqId + ", time: " + Util.UnixSecondsToString(time,
        "yyyyMMdd-HH:mm:ss") + ", price: " + Util.DoubleMaxString(price) + ", size: "
        + size + ", exchange: " + exchange + ", special conditions:" + specialConditions
        + ", tick attribs: " + (tickAttribLast.pastLimit() ? "pastLimit " : "") + (
        tickAttribLast.unreported() ? "unreported " : "");
  }

  public static String tickByTickAllLast(int reqId, int tickType, long time, double price,
      Decimal size, TickAttribLast tickAttribLast,
      String exchange, String specialConditions) {
    return (tickType == 1 ? "Last." : "AllLast.") +
        " Req Id: " + reqId + " Time: " + Util.UnixSecondsToString(time, "yyyyMMdd-HH:mm:ss")
        + " Price: " + Util.DoubleMaxString(price) + " Size: " + size +
        " Exch: " + exchange + " Spec Cond: " + specialConditions + " Tick Attibs: " + (
        tickAttribLast.pastLimit() ? "pastLimit " : "") +
        (tickType == 1 ? "" : (tickAttribLast.unreported() ? "unreported " : ""));
  }

  public static String tickByTickBidAsk(int reqId, long time, double bidPrice, double askPrice,
      Decimal bidSize, Decimal askSize,
      TickAttribBidAsk tickAttribBidAsk) {
    return "BidAsk. Req Id: " + reqId + " Time: " + Util.UnixSecondsToString(time,
        "yyyyMMdd-HH:mm:ss") + " BidPrice: " + Util.DoubleMaxString(bidPrice) +
        " AskPrice: " + Util.DoubleMaxString(askPrice) + " BidSize: " + bidSize + " AskSize: "
        + askSize + " Tick Attibs: " +
        (tickAttribBidAsk.bidPastLow() ? "bidPastLow " : "") + (tickAttribBidAsk.askPastHigh()
        ? "askPastHigh " : "");
  }

  public static String tickByTickMidPoint(int reqId, long time, double midPoint) {
    return "MidPoint. Req Id: " + reqId + " Time: " + Util.UnixSecondsToString(time,
        "yyyyMMdd-HH:mm:ss") + " MidPoint: " + Util.DoubleMaxString(midPoint);
  }

  public static String orderBound(long orderId, int apiClientId, int apiOrderId) {
    return "order bound: apiOrderId=" + Util.IntMaxString(apiOrderId) + " apiClientId="
        + Util.IntMaxString(apiClientId) + " permId=" + Util.LongMaxString(orderId);
  }
  ...
  public static String historicalSchedule(int reqId, String startDateTime, String endDateTime,
  ...
  private static void appendOrderFields(StringBuilder sb, int orderId, Contract contract,
    ...
    if (order.volatility() != Double.MAX_VALUE) {
      ...
      Util.appendNonEmptyString(sb, "deltaNeutralClearingAccount",
          order.deltaNeutralClearingAccount());
      Util.appendNonEmptyString(sb, "deltaNeutralClearingIntent",
      ...
      if (order.deltaNeutralShortSale()) {
        Util.appendValidIntValue(sb, "deltaNeutralShortSaleSlot",
            order.deltaNeutralShortSaleSlot());
        Util.appendNonEmptyString(sb, "deltaNeutralDesignatedLocation",
    ...
    if ("PEG BENCH".equals(order.getOrderType())) {
      ...
      Util.appendBooleanFlag(sb, "isPeggedChangeAmountDecrease",
      ...
      if (contract.comboLegs() != null) {
        for (int i = 0; i < contract.comboLegs().size(); ++i) {
          ...
          if (order.orderComboLegs() != null
              && contract.comboLegs().size() == order.orderComboLegs().size()) {
    ...
    if (order.competeAgainstBestOffset() != Double.MAX_VALUE) {
      if (order.isCompeteAgainstBestOffsetUpToMid()) {
        ...
        sb.append(Util.SPACE_SYMBOL).append("competeAgainstBestOffset").append(Util.EQUALS_SIGN)
            .append(order.competeAgainstBestOffset());
      }
    }
